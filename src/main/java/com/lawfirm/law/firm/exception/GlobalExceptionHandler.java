 ...
 *
 *  1. VALIDAÇÃO (400) - requisição malformada ou campo inválido. Não é logada
 *     como problema (é comportamento normal da API).
 *  2. NEGÓCIO (404/409/422) - requisição válida, mas o domínio não permite:
 *     recurso inexistente, duplicidade, regra violada. Logada como WARN.
 *  3. SISTEMA (500) - falha de infraestrutura ou bug. Logada como ERROR com
 *     stack trace; a resposta NUNCA expõe detalhe técnico, só o código.
 *
 * Toda resposta de erro usa o envelope padrão {@link ApiResponse} com a lista
 * de {@link ApiError} (field, message, code).
 */
...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ApiResponse<Void>> handleBeanValidation(MethodArgumentNotValidException ex) {
        List<ApiError> errors = ex.getBindingResult().getFieldErrors().stream()
                .map(fe -> {
                    ...
                    return new ApiError(fe.getField(), humanize(label, fe.getDefaultMessage()), "VALIDATION_ERROR");
                })
                .toList();
    ...
    public ResponseEntity<ApiResponse<Void>> handleConstraint(ConstraintViolationException ex) {
        List<ApiError> errors = ex.getConstraintViolations().stream()
                .map(cv -> {
                    ...
                    return new ApiError(field, humanize(label, cv.getMessage()), "VALIDATION_ERROR");
                })
                .toList();
        return respond(HttpStatus.BAD_REQUEST, errors);
    }
    ...
    public ResponseEntity<ApiResponse<Void>> handleCustomValidation(ValidationException ex) {
        String code = ex.getValidationErrorCode() != null ? ex.getValidationErrorCode().getCode() : ex.getCode();
        return respond(HttpStatus.BAD_REQUEST, List.of(new ApiError(ex.getField(), ex.getMessage(), code)));
    }

    @ExceptionHandler(MethodArgumentTypeMismatchException.class)
    public ResponseEntity<ApiResponse<Void>> handleTypeMismatch(MethodArgumentTypeMismatchException ex) {
        String message = "Valor inválido para o parâmetro '" + ex.getName() + "': " + ex.getValue();
        return respond(HttpStatus.BAD_REQUEST, List.of(new ApiError(ex.getName(), message, "INVALID_PARAMETER")));
    }
    ...
    public ResponseEntity<ApiResponse<Void>> handleMissingPart(Exception ex) {
        ...
        return respond(HttpStatus.BAD_REQUEST,
                List.of(new ApiError(name, "Parâmetro obrigatório ausente: " + name, "REQUIRED_FIELD")));
    }

    @ExceptionHandler(HttpMessageNotReadableException.class)
    public ResponseEntity<ApiResponse<Void>> handleUnreadable(HttpMessageNotReadableException ex) {
        String msg = ex.getMostSpecificCause() != null ? ex.getMostSpecificCause().getMessage() : ex.getMessage();
        return respond(HttpStatus.BAD_REQUEST, List.of(new ApiError(null, msg, "INVALID_REQUEST_BODY")));
    }

    @ExceptionHandler(MaxUploadSizeExceededException.class)
    public ResponseEntity<ApiResponse<Void>> handleMaxUploadSize(MaxUploadSizeExceededException ex) {
        return respond(HttpStatus.BAD_REQUEST, List.of(new ApiError("files",
                "Arquivo(s) excedem o tamanho máximo permitido para upload.", "FILE_TOO_LARGE")));
    }
    ...
    public ResponseEntity<ApiResponse<Void>> handleNotFound(NotFoundException ex) {
        return respond(HttpStatus.NOT_FOUND, List.of(new ApiError(null, ex.getMessage(), "NOT_FOUND")));
    }

    @ExceptionHandler(BusinessException.class)
    public ResponseEntity<ApiResponse<Void>> handleBusiness(BusinessException ex) {
        log.warn("Regra de negócio violada [{}]: {}", ex.getCode(), ex.getMessage());
        return respond(HttpStatus.UNPROCESSABLE_ENTITY, List.of(new ApiError(null, ex.getMessage(), ex.getCode())));
    }
    ...
    public ResponseEntity<ApiResponse<Void>> handleAccessDenied(AccessDeniedException ex) {
        return respond(HttpStatus.FORBIDDEN, List.of(new ApiError(null, "Acesso negado", "ACCESS_DENIED")));
    }

    @ExceptionHandler(NoResourceFoundException.class)
    public ResponseEntity<ApiResponse<Void>> handleNoResource(NoResourceFoundException ex) {
        return respond(HttpStatus.NOT_FOUND,
                List.of(new ApiError(null, "Rota não encontrada: " + ex.getResourcePath(), "NOT_FOUND")));
    }
    ...
    public ResponseEntity<ApiResponse<Void>> handleSystem(SystemException ex) {
        ...
        return respond(HttpStatus.INTERNAL_SERVER_ERROR, List.of(new ApiError(null, message, ex.getCode())));
    }
    ...
    public ResponseEntity<ApiResponse<Void>> handleFileStorage(FileStorageException ex) {
        ...
        return respond(HttpStatus.INTERNAL_SERVER_ERROR,
                List.of(new ApiError(null, SystemErrorCode.FILE_STORAGE_ERROR.getMessage(),
    ...
    private static String humanize(String label, String defaultMsg) {
        ...
        if (msg.contains("not be blank") || msg.contains("not be null")
                || msg.contains("não pode estar em branco") || msg.contains("não pode ser nulo")) {
