...
/**
 * Collection única de arquivos do cliente, dividida em duas sub-coleções que
 * espelham as abas da tela: /files/documents e /files/simulations.
 */
@Tag(name = "Arquivos do cliente",
        description = "Documentos (11 tipos) e simulações do cliente - upload, listagem paginada, download, "
...
public class ClientFileController {
    ...
    @Operation(summary = "Upload em lote de documentos",
            description = "Recebe N arquivos (PDF/PNG/JPEG) na parte 'files' e uma lista de metadados na mesma "
    ...
    public ResponseEntity<ApiResponse<ClientFileDocumentResponseDTO>> uploadDocuments(
        ...
        List<ClientFileDocumentResponseDTO> created = service.uploadDocuments(clientId, files, metadata);
        return ResponseEntity.status(HttpStatus.CREATED).body(ApiResponse.successList(created));
    }

    @Operation(summary = "Listar documentos",
            description = "Documentos ativos do cliente, mais recente primeiro, paginados no envelope padrão. "
    ...
    public ResponseEntity<ApiResponse<ClientFileDocumentResponseDTO>> listDocuments(
        ...
        Page<ClientFileDocumentResponseDTO> page = service.listDocuments(clientId, documentType, pageNumber, pageSize);
    ...
    public ResponseEntity<Resource> downloadDocument(@PathVariable UUID clientId, @PathVariable UUID fileId) {
        return toDownloadResponse(service.downloadDocument(clientId, fileId));
    }

    @Operation(summary = "Atualizar metadados de um documento",
            description = "Atualização parcial: permite trocar o tipo do documento e/ou as observações. "
    ...
    public ResponseEntity<ApiResponse<ClientFileDocumentResponseDTO>> updateDocument(
        ...
        return ResponseEntity.ok(ApiResponse.successObject(service.updateDocument(clientId, fileId, dto)));
    }

    @Operation(summary = "Excluir um documento", description = "Soft delete - o arquivo é preservado como evidência.")
    @DeleteMapping("/documents/{fileId}")
    public ResponseEntity<Void> deleteDocument(@PathVariable UUID clientId, @PathVariable UUID fileId) {
    ...
    @Operation(summary = "Upload em lote de simulações",
            description = "Recebe N arquivos PDF na parte 'files' e uma lista de metadados na mesma ordem na "
    ...
    public ResponseEntity<ApiResponse<ClientFileSimulationResponseDTO>> uploadSimulations(
        ...
        List<ClientFileSimulationResponseDTO> created = service.uploadSimulations(clientId, files, metadata);
        return ResponseEntity.status(HttpStatus.CREATED).body(ApiResponse.successList(created));
    }

    @Operation(summary = "Listar simulações",
            description = "Simulações ativas do cliente, mais recente primeiro, paginadas no envelope padrão.")
    @GetMapping("/simulations")
    public ResponseEntity<ApiResponse<ClientFileSimulationResponseDTO>> listSimulations(
            @PathVariable UUID clientId,
            @Parameter(description = "Número da página (1-based)") @RequestParam(defaultValue = "1") int pageNumber,
            @Parameter(description = "Tamanho da página") @RequestParam(defaultValue = "10") int pageSize) {
        Page<ClientFileSimulationResponseDTO> page = service.listSimulations(clientId, pageNumber, pageSize);
        return ResponseEntity.ok(ApiResponse.successList(page.getContent(), Pagination.of(page)));
    }
    ...
    public ResponseEntity<ApiResponse<ClientFileSimulationResponseDTO>> getSimulation(
            @PathVariable UUID clientId, @PathVariable UUID fileId) {
        return ResponseEntity.ok(ApiResponse.successObject(service.getSimulation(clientId, fileId)));
    }

    @Operation(summary = "Download de uma simulação")
    @GetMapping("/simulations/{fileId}/download")
    public ResponseEntity<Resource> downloadSimulation(@PathVariable UUID clientId, @PathVariable UUID fileId) {
        return toDownloadResponse(service.downloadSimulation(clientId, fileId));
    }

    @Operation(summary = "Atualizar metadados de uma simulação",
            description = "Atualização parcial de data da simulação, versão (texto livre, ex.: v1.0.2), "
    ...
    public ResponseEntity<ApiResponse<ClientFileSimulationResponseDTO>> updateSimulation(
        ...
        return ResponseEntity.ok(ApiResponse.successObject(service.updateSimulation(clientId, fileId, dto)));
    }

    @Operation(summary = "Marcar simulação como principal",
            description = "Marca esta simulação como a principal do cliente, desmarcando a anterior.")
    @PatchMapping("/simulations/{fileId}/principal")
    public ResponseEntity<ApiResponse<ClientFileSimulationResponseDTO>> markPrincipal(
            @PathVariable UUID clientId, @PathVariable UUID fileId) {
        return ResponseEntity.ok(ApiResponse.successObject(service.markSimulationPrincipal(clientId, fileId)));
    }

    @Operation(summary = "Excluir uma simulação",
            description = "Soft delete. Se a simulação excluída era a principal, a mais recente restante é promovida.")
    @DeleteMapping("/simulations/{fileId}")
    public ResponseEntity<Void> deleteSimulation(@PathVariable UUID clientId, @PathVariable UUID fileId) {
    ...
    private static ResponseEntity<Resource> toDownloadResponse(FileDownload download) {
        ...
        headers.setContentDisposition(ContentDisposition.attachment()
                .filename(download.originalFilename() != null ? download.originalFilename() : "arquivo")
                .build());
